    ...
    : IRequestHandler<GetHivesByApiaryQuery, Result<IReadOnlyList<HiveDto>>>
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return Result<IReadOnlyList<HiveDto>>.Failure("User is not authenticated.");
        }

        var apiary = await _apiaryRepository.GetByIdAsync(request.ApiaryId, cancellationToken);
        if (apiary is null)
        {
            return Result<IReadOnlyList<HiveDto>>.Failure("Apiary was not found.");
        }

        if (apiary.BeekeeperId != beekeeperId)
        {
            return Result<IReadOnlyList<HiveDto>>.Failure("Apiary does not belong to the current beekeeper.");
        }
